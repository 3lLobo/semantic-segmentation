...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        # Creater smaller batches of images
        for images, labels in get_batches_fn(batch_size):
            x, loss = sess.run([train_op, cross_entropy_loss], feed_dict={learning_rate: LEARNING_RATE, keep_prob: KEEP_PROB, input_image:images, correct_label:labels})

            # Add current loss to list
            loss_history.append(loss)
        print("EPOCH {} ...".format(epoch + 1))
        print("Traing loss = {:.3f}".format(loss))
...
def run():
    ...
    with tf.Session() as sess:
		...
		#logits, loss_op, train_op = optimize(layer_output, label, learning_rate, num_classes )

        logits, cross_entropy_loss, train_op = optimize(layer_output, label, learning_rate, num_classes )
		
        loss_history = train_nn(sess, EPOCHS, BATCH_SIZE, get_batches_fn, train_op, cross_entropy_loss, input_image, label, keep_prob, learning_rate)
